...
router.get("/pacientes/nome/:nome", (req, res) => {
  const nome = req.params.nome.toLowerCase();

  // Busca parcial
  const paciente = pacientes.find(p => p.nome.toLowerCase().includes(nome));
...
router.put("/pacientes/id/:id", (req, res) => {
...
router.delete("/pacientes/id/:id", (req, res) => {
  const index = pacientes.findIndex(p => p.id === parseInt(req.params.id));
  if (index === -1) {
    return res.status(404).json({ error: "Paciente não encontrado" });
  }
